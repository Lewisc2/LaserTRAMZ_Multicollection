...
class define_analyses(param.Parameterized):
    ...
    add_reduction_button = param.Action(lambda x: x.add_reduction(), label='Add Interval',precedence=0) # button that addes current interval (background + ablation) to get reduced. Triggers modal to name analysis
    ...
    export_data_button = param.Action(lambda x: x.export_stored_intervals(), label='Output Selected Intervals')
    ...
    def accept_array(self,event=None):
        ...
        self.stored_intervals_data.insert(1,'SampleLabel','SampleLabels')
    ...
    def store_interval(self,event=None):
        ...
        if self.stored_intervals_data['SampleLabel'].str.contains(str(sample_name)).any():
            duplicates = self.stored_intervals_data[self.stored_intervals_data['SampleLabel'].str.contains(sample_name)]
        ...
        new_interval_df['SampleLabel'] = updated_sample_name
        ...
        self.stored_intervals_data['measurementindex'] = self.stored_intervals_data.sort_values(by='Analysis Start').index
    
        fastgrid_layout.close_modal()
        
    @pn.depends('stored_intervals_data')
    def export_stored_intervals(self,event=None):
        output_df = self.stored_intervals_data
        output_df = output_df.drop(0,axis=0)
        
        output_df.to_excel('LaserTRAMZMC_SelectedIntervals.xlsx',index=False)
...
widgets={'analytes_': pn.widgets.CheckBoxGroup,
         ...
         'export_data_button': pn.widgets.Button(name='Export', button_type='success'),
         }
