...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 21 07:35:26 2025
...
# %%
class calc_fncs_reduceintervals:
    ...
    def get_counts(data,counts_mode,ablation_start_true,bckgrnd_start_input,bckgrnd_stop_input,ablation_start_input,ablation_stop_input, integration_time,analyte_cols):
        ...
        if counts_mode == 'Total Counts':
            ...
            data_ratios_ = calc_fncs_reduceintervals.get_ratios(data) # get ratios
            data_ratios_ = calc_fncs_reduceintervals.threesig_outlierremoval(data_ratios_)
            ...
            data_ratios_ = calc_fncs_reduceintervals.get_ratios(data)
            data_ratios_ = calc_fncs_reduceintervals.threesig_outlierremoval(data_ratios_)
    ...
    def get_regressions(data,regression_buttons,ablation_start_true):
        ...
        """
        data = calc_fncs_reduceintervals.threesig_outlierremoval(data)
    ...
    def get_approved(data,bckgrnd_start_input,bckgrnd_stop_input,
        ...
        if counts_mode == 'Total Counts':
            data_ratios_all,data_stats_all = calc_fncs_reduceintervals.get_counts(data_toapprove, counts_mode, ablation_start_true, bckgrnd_start_input, bckgrnd_stop_input, ablation_start_input, ablation_stop_input,integration_time,analyte_cols)
            ...
            data_approved_ratio = calc_fncs_reduceintervals.get_ratios(data_approved_ratio) # get calculated ratios of the background subtracted data. Need 0 and not 'bdl' for calcs to pass
        
            data_ratios_reg,data_stats_reg = calc_fncs_reduceintervals.get_regressions(data_approved_ratio,regression_buttons,ablation_start_true) # get estimated regression intercepts, background subtracted ratios, regression statistics, and SE's of the ratios3
            data_ratios_counts,data_stats_counts = calc_fncs_reduceintervals.get_counts(data_toapprove, counts_mode, ablation_start_true, bckgrnd_start_input, bckgrnd_stop_input, ablation_start_input, ablation_stop_input,integration_time,analyte_cols)
        ...
        if ellipsemode_selector is True:
            ...
            ellipse_data_toapprove = calc_fncs_reduceintervals.get_ratios(ellipse_data_toapprove)
        ...
        Weth_ellparams,TW_ellparams = calc_fncs_reduceintervals.get_ellipse(ellipse_data_toapprove,power,ablation_start_true,regression_buttons,counts_mode)
    ...
    def get_ellipse(data,power,ablation_start_true,regression_buttons,counts_mode):
        ...
        if counts_mode != 'Total Counts':
            predicted,predicted_207 = calc_fncs_reduceintervals.get_regressions(data,regression_buttons,ablation_start_true)
...
# %%
class plots_reduceintervals(calc_fncs_reduceintervals):
    """ Class that holds all of the functions for reducing the time resolved data"""
    def __init__(self,*args):
        super().__init__(*args)
        for a in args:
    ...
    def ratio_plot(data,ratio_buttons,regression_buttons,
        ...
        """
        data = calc_fncs_reduceintervals.get_ratios(data) # get calculated ratios from the data
        ...
        if displayframe == 'main':
            ...
            regressions,stats = calc_fncs_reduceintervals.get_regressions(data_to_regress,regression_buttons,ablation_start_true)
    ...
    def residuals_plot(data,regression_buttons,start_ablation,stop_ablation,ablation_start_true):
        ...
        fig207 = figure(height=250,width=250,title='207Pb/235U Residuals',tools='pan,reset,save,wheel_zoom,xwheel_zoom,ywheel_zoom',toolbar_location='above',
                        x_axis_label='Fitted Value',y_axis_label='Residuals')
        data = calc_fncs_reduceintervals.get_ratios(data) # calculate relevant isotopic ratios from the data
        data_to_regress = data # reassign variable name for clarity
        # get fitted regression values, regression names, and residuals
        fitted,fitted207,residuals,residuals207 = calc_fncs_reduceintervals.get_regressions(data_to_regress,regression_buttons,ablation_start_true)
    ...
    def ellipse_plot(data,power,start_ablation,stop_ablation,ablation_start_true,regression_buttons,counts_mode):
        ...
        """
        data = calc_fncs_reduceintervals.get_ratios(data)
        data = data.dropna()
        ...
        ell1p,ell2p = calc_fncs_reduceintervals.get_ellipse(data, power,ablation_start_true,regression_buttons,counts_mode) # get the parameters of the confidence ellipsoids
...
class define_analyses(param.Parameterized):
    ...
    def _uploadfile(self):
        ...
        if self.file_path != 'Insert File Path':
            ...
            for n in range(n_analytes):
                ...
                fastgrid_layout_selectintervals.modal[rth_row].append(pn.Column(pn.Row(pn.widgets.TextInput(placeholder='Mass')),
                                                                pn.Row(pn.widgets.RadioButtonGroup(options=['Volts','Counts']))))
                nth_iter = nth_iter + 1
                if nth_iter % 3 == 0:
                    rth_row = rth_row + 1
            fastgrid_layout_selectintervals.modal[-1].append(pn.Column(buttons_))
            fastgrid_layout_selectintervals.open_modal()
    ...
    def accept_array(self,event=None):
        ...
        added_analytes_list = []
        for n in range(n_analytes):
            next_analyte = fastgrid_layout_selectintervals.modal[rth_row][nth_iter][0][0].value
            detector_type = fastgrid_layout_selectintervals.modal[rth_row][nth_iter][1][0].value
        ...
        fastgrid_layout_selectintervals.close_modal()
    ...
    def call_simple_ablation_plot(self):
        ...
        if self.stored_intervals_data is not None:
            ...
            return pn.pane.Bokeh(row(select_intervals_plots.simple_ablation_plot(data_toplot,self.analysis_start,self.analysis_end,self.logcountsdata,self.analytes_)))
    ...
    def add_reduction(self, event=None):
        ...
        """      
        # clear current modal
        fastgrid_layout_selectintervals.modal[0].clear()
        fastgrid_layout_selectintervals.modal[1].clear()
        fastgrid_layout_selectintervals.modal[2].clear()
        fastgrid_layout_selectintervals.modal[3].clear()
        fastgrid_layout_selectintervals.modal[4].clear()
        
        # put button and text box on modal for recording sample name
        fastgrid_layout_selectintervals.modal[0].append(store_interval_widget_button) # this needs to be a button widget implemented when calling class instance that is linked to the store_interval_button above
        fastgrid_layout_selectintervals.modal[1].append(pn.widgets.TextInput(placeholder='Enter Sample Name'))
    ...
    def store_interval(self,event=None):
        ...
        sample_name = fastgrid_layout_selectintervals.modal[1][0].value
...
class make_reduction_plots(param.Parameterized):
    ...
    def call_ablation_plot(self):
        ...
        if self.output_data is not None:
            data_toplot = self.input_data
            return pn.pane.Bokeh(row(plots_reduceintervals.ablation_plot(data_toplot,self.background_start,self.background_end,
    ...
    def call_ratio_plot(self):
        ...
        if self.output_data is not None:
            ...
            return pn.pane.Bokeh(row(plots_reduceintervals.ratio_plot(data_toplot,self.ratio_buttons,self.regression_buttons,self.background_start,self.background_end,
    ...
    def call_ratio_plot76(self):
        ...
        if self.output_data is not None:
            if self.lock_ablation_start_true == True:
                self.ablation_start_true = self.ablation_start
            data_toplot = self.input_data[(self.input_data['Time'] >= self.background_start-3) & (self.input_data['Time'] <= self.ablation_end+10)]
            return pn.pane.Bokeh(row(plots_reduceintervals.ratio_plot_76(data_toplot,
    ...
    def evaluate_output_data(self, event=None):
        ...
        ratioplot_pane = pn.pane.Bokeh(row(plots_reduceintervals.ratio_plot(data_ratio_plot,self.ratio_buttons,self.regression_buttons,self.background_start,self.background_end,
        ...
        residuals_plot_pane = pn.pane.Bokeh(row(plots_reduceintervals.residuals_plot(data_residuals_plot,self.regression_buttons,self.ablation_start,self.ablation_end,self.ablation_start_true)))
        
        Weth_ell,TW_ell = plots_reduceintervals.ellipse_plot(data_residuals_plot, self.power, self.ablation_start, self.ablation_end, self.ablation_start_true, self.regression_buttons,self.counts_mode)
        ellipse_tabs = pn.Tabs(('TW',TW_ell),('Weth.',Weth_ell),dynamic=True)
        
        
        datastats = calc_fncs_reduceintervals.get_ratios(data_residuals_plot)
        datastats = datastats.reset_index(drop=True)
        regressions,stats = calc_fncs_reduceintervals.get_regressions(datastats,self.regression_buttons,self.ablation_start_true)
        ...
        fastgrid_layout_reduceintervals.modal[0].clear()
        fastgrid_layout_reduceintervals.modal[1].clear()
        fastgrid_layout_reduceintervals.modal[2].clear()
        fastgrid_layout_reduceintervals.modal[3].clear()
        fastgrid_layout_reduceintervals.modal[4].clear()
        
        fastgrid_layout_reduceintervals.modal[0].append(buttons_sample)
        fastgrid_layout_reduceintervals.modal[1].append(pn.widgets.TextInput(placeholder='Enter Sample Name'))
        fastgrid_layout_reduceintervals.modal[2].append(pn.Row(ratioplot_pane,pn.Spacer(width=50, height=500),residuals_plot_pane))
        fastgrid_layout_reduceintervals.modal[3].append(pn.Row(ellipse_tabs,pn.Spacer(width=50, height=500),stats_markdown))
        
        fastgrid_layout_reduceintervals.open_modal()
    ...
    def send_reduction(self,event=None):
        ...
        """
        new_index = 0
        sample_name = fastgrid_layout_reduceintervals.modal[1][0].value
        ...
        data_approved = calc_fncs_reduceintervals.get_approved(data_tosend,self.background_start,self.background_end,self.ablation_start,self.ablation_end,
        ...
        fastgrid_layout_reduceintervals.close_modal()
...
# %%
def select_intervals_page():
    callapp_selectintervals = define_analyses(name='Define Analysis Intervals')

    buttons_=pn.WidgetBox(pn.Param(callapp_selectintervals.param.accept_array_button,widgets={'accept_array_button': pn.widgets.Button(name='Accept Detector Array',button_type='success')}))
    store_interval_widget_button=pn.WidgetBox(pn.Param(callapp_selectintervals.param.store_interval_button,widgets={'store_interval_button': pn.widgets.Button(name='Store Interval',button_type='success')}))
    ...
    fastgrid_layout_selectintervals = pn.template.FastListTemplate(title='LaserTRAMZ: LA-MC-ICP-MS Select Intervals',
                                                                  sidebar=pn.Column(pn.WidgetBox(pn.Param(callapp_selectintervals.param,widgets=widgets))),sidebar_width=380,
                                                                  )
    
    fastgrid_layout_selectintervals.modal.append(pn.Row())
    fastgrid_layout_selectintervals.modal.append(pn.Row())
    fastgrid_layout_selectintervals.modal.append(pn.Row())
    fastgrid_layout_selectintervals.modal.append(pn.Row())
    fastgrid_layout_selectintervals.modal.append(pn.Row())
    
    fastgrid_layout_selectintervals.main.append(pn.Row(pn.WidgetBox(pn.Param(callapp_selectintervals.param.analysis_start,widgets={'analysis_start': pn.widgets.EditableFloatSlider(start=0,end=8600,value=(51.1),step=1,name='Analysis Start',width=1300,height=20)},),
                                                                    pn.Param(callapp_selectintervals.param.analysis_end,widgets={'analysis_end': pn.widgets.EditableFloatSlider(start=0,end=8600,value=(76.3),step=1,name='Analysis End',width=1300,height=20),}),
                                                                    width=1400,height=180
                                                                    )
                                                       )
                                                )
    
    fastgrid_layout_selectintervals.main.append(pn.Column(callapp_selectintervals.call_simple_ablation_plot))
    fastgrid_layout_selectintervals.main.append(pn.Column(callapp_selectintervals._update_stored_intervals_widget))
    fastgrid_layout_selectintervals.servable()
    return fastgrid_layout_selectintervals()

# fastgrid_layout_selectintervals.show();
...
def reduce_intervals_page():
    callapp_reduceintervals = make_reduction_plots(name='Reduce Ablation Data')
    
    buttons_sample=pn.WidgetBox(pn.Param(callapp_reduceintervals.param.accept_interval_button,
    ...
    fastgrid_layout_reduceintervals = pn.template.FastListTemplate(title='LaserTRAMZ: LA-MC-ICP-MS',
                                                                  sidebar=pn.Column(pn.WidgetBox(pn.Param(callapp_reduceintervals.param,widgets=widgets))),sidebar_width=380)
    
    fastgrid_layout_reduceintervals.modal.append(pn.Row())
    fastgrid_layout_reduceintervals.modal.append(pn.Row())
    fastgrid_layout_reduceintervals.modal.append(pn.Row())
    fastgrid_layout_reduceintervals.modal.append(pn.Row())
    fastgrid_layout_reduceintervals.modal.append(pn.Row())
    
    fastgrid_layout_reduceintervals.main.append(pn.Row(pn.WidgetBox(pn.Param(callapp_reduceintervals.param.ablation_start,widgets={'ablation_start': pn.widgets.EditableFloatSlider(start=0,end=8600,value=(51.1),step=1,name='Ablation Start',width=1300,height=20)},),
                                                    pn.Param(callapp_reduceintervals.param.ablation_end,widgets={'ablation_end': pn.widgets.EditableFloatSlider(start=0,end=8600,value=(76.3),step=1,name='Ablation End',width=1300,height=20),}),
                                                    pn.Param(callapp_reduceintervals.param.background_start,widgets={'background_start': pn.widgets.EditableFloatSlider(start=0,end=8600,value=(31.2),step=1,name='Background Start',width=1300,height=20),}),
                                                    pn.Param(callapp_reduceintervals.param.background_end,widgets={'background_end': pn.widgets.EditableFloatSlider(start=0,end=8600,value=(46.3),step=1,name='Background End',width=1300,height=20)}),
                                                    width=1400,height=180
                                                    )
                                       )
                                ) # for vanilla
    fastgrid_layout_reduceintervals.main.append(pn.Column(callapp_reduceintervals.call_ablation_plot,pn.Row(callapp_reduceintervals.call_ratio_plot,callapp_reduceintervals.call_ratio_plot76))) # for vanilla
    fastgrid_layout_reduceintervals.main.append(pn.Column(callapp_reduceintervals._update_output_widget))

# fastgrid_layout_reduceintervals.show();
    fastgrid_layout_reduceintervals.servable()
    return fastgrid_layout_reduceintervals()

Pages = {"Select Intervals": select_intervals_page, "Reduce Intervals": reduce_intervals_page}

pn.serve(Pages,port=5007)
...
# Pages = {"Select Intervals": fastgrid_layout_selectintervals, "Reduce Intervals": fastgrid_layout_reduceintervals}

# pn.serve(Pages,port=5006)
